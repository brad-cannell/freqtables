...
#'
#' This version is soft deprecating ..., but ... still works.
#'
...
freq_table_v2 <- function(.data, ..., .x, percent_ci = 95, ci_type = "logit", drop = FALSE) {
  ...
  #     (https://devguide.ropensci.org/evolution.html#parameters-changing-parameter-names),
  #     we will give a warning about passing multiple columns to the ellipsis
  #     argument.
  #   - In about a year, we will remove ... and only allow .x
  # ===========================================================================
  dots_length <- length(enquos(...))
  if (dots_length > 0) {
    message(
      "Using the `...` argument to pass multiple column names to freq_table is ",
      ...
      "the freq_table's `.x` argument. In future versions of freq_table, passing ",
      "column names to `...` will not be supported. If you were trying to ",
      "the value of percent_ci passing a value by position, please pass the ",
      "value by name (e.g., `percent_ci = 99). Use the `.x` argument to avoid ",
      "this message."
    )
  }
  ...
  # Because ... is soft deprecated, we need to versions of code to get within
  # group counts. Only the second version will remain after ... is removed
  # completely.
  # ===========================================================================
  if (missing(.x)) {
...
# data(mtcars)
# devtools::load_all()
# mtcars %>% freq_table_v2()
# mtcars %>% freq_table_v2(cyl)
# mtcars %>% group_by(am) %>% freq_table_v2(cyl)
# mtcars %>% group_by(am) %>% freq_table_v2(.x = cyl)
# mtcars %>% group_by(am) %>% freq_table_v2(cyl, vs)
# mtcars %>% group_by(am) %>% freq_table_v2(cyl, "wald")
